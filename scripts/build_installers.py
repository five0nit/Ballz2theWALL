...
def build(wheel: Path, output: Path) -> list[dict]:
    ...
    for platform, launcher in (("Windows", "Install-Windows.cmd"), ("Mac", "Install-Mac.command")):
        members = {launcher: (root / "installers" / launcher).read_bytes(),
                   "payload/" + wheel.name: wheel.read_bytes(),
        ...
        instructions = (
            ...
            "An installed Hermes, OpenAI Codex or Claude Code agent is required; it handles its own sign-in.\n"
            ...
            "This local alpha is not a signed/notarized public installer. If your OS blocks it, do not expect the\n"
        ...
        with zipfile.ZipFile(destination) as archive:
            ...
            assert archive.read("payload/" + wheel.name) == wheel.read_bytes()
            if platform == "Mac":
